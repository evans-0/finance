...
import { C, MONO, Counter, Section } from './shared'
            ...
            <div key={step.id} style={{ display: 'flex', alignItems: 'center' }}>
              ...
              {i < FLOW_STEPS.length - 1 && (
                <div style={{ display: 'flex', alignItems: 'center', flexShrink: 0, width: 32 }}>
                  ...
                  <div style={{ fontSize: 10, color: active > i ? C.green : C.border, lineHeight: 1, marginLeft: -1 }}>▶</div>
                </div>
              )}
            </div>
          ))}
...
function OrderBook() {
  const asks = [{ price: 302.50, qty: 450 }, { price: 302.25, qty: 230 }, { price: 302.00, qty: 890 }]
  const bids = [{ price: 301.75, qty: 1200 }, { price: 301.50, qty: 670 }, { price: 301.25, qty: 340 }]
...
function SupplyDemand() {
  ...
  return (
    <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 4, padding: 24 }}>
      ...
      <input type="range" min={10} max={90} value={buyers} onChange={e => setBuyers(Number(e.target.value))} style={{ width: '100%', accentColor: C.amber }} />
...
const ORDER_TYPES = [
  { type: 'Market Order', icon: '⚡', color: C.amber, desc: 'Buy or sell immediately at the best available price.', pros: ['Guaranteed execution', 'Instant fill'], cons: ['Price not guaranteed', 'Bad in volatile markets'], example: '"Buy 10 shares of AAPL right now at whatever price it\'s at."' },
  { type: 'Limit Order',  icon: '🎯', color: C.blue,  desc: 'Buy or sell only at a specific price or better.',     pros: ['Price guaranteed', 'No surprises'],          cons: ['May not execute', 'Can miss the trade'],           example: '"Buy 10 shares of AAPL only if the price drops to $290."' },
  { type: 'Stop Loss',    icon: '🛡️', color: C.red,   desc: 'Automatically sell if the price falls to a set level.', pros: ['Limits downside', 'Hands-off protection'],  cons: ['Can trigger on temporary dips', 'Guaranteed exit, not price'], example: '"Sell my AAPL if it drops to $285 to limit my loss."' },
]

const PARTICIPANTS = [
  { name: 'Retail Investors',  icon: '👤', pct: 45, color: C.amber,  desc: 'Individual investors like you. Buy and hold or trade for their own account.' },
  { name: 'FIIs',              icon: '🌍', pct: 25, color: C.blue,   desc: 'Foreign Institutional Investors — overseas funds. Their movements heavily influence Nifty.' },
  { name: 'DIIs',              icon: '🏛️', pct: 20, color: C.green,  desc: 'Domestic Institutional Investors — Indian mutual funds, insurance, pension funds.' },
  { name: 'HNIs / Prop Desks', icon: '💼', pct: 10, color: C.purple, desc: 'High Net Worth Individuals and prop desks. Often algorithmic, trade large volumes.' },
]

const HOURS = [
  { time: '9:00',  label: 'Pre-Open',      sub: 'Orders collected, opening price discovered', color: C.textSec },
  { time: '9:15',  label: 'Market Opens',  sub: 'Regular trading begins',                     color: C.green   },
  { time: '3:30',  label: 'Market Closes', sub: 'No new orders accepted',                     color: C.red     },
  { time: '3:40',  label: 'Post-Close',    sub: 'Closing price calculated',                   color: C.textSec },
  { time: 'T+1',   label: 'Settlement',    sub: 'Shares delivered, money transferred',        color: C.amber   },
]
    ...
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: MONO, color: C.text, overflowX: 'hidden' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: 'clamp(24px, 4vw, 52px) clamp(12px, 3vw, 24px)' }}>
        ...
        <div style={{ marginTop: 24, marginBottom: 48 }}>
          ...
          <h1 style={{ fontSize: 32, fontWeight: 700, marginBottom: 16, lineHeight: 1.2 }}>Equity Market</h1>
            ...
            {HOURS.map((h, i) => (
              <div key={h.time} style={{ display: 'flex', gap: 16, alignItems: 'stretch' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 40, flexShrink: 0 }}>
                  <div style={{ width: 16, height: 16, borderRadius: 8, background: h.color === C.green ? C.green : C.bg, border: `2px solid ${h.color}`, flexShrink: 0, marginTop: 4 }} />
                  {i < HOURS.length - 1 && <div style={{ width: 1, flex: 1, background: C.border, minHeight: 20, margin: '4px 0' }} />}
                </div>
