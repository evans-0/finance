...
function DerivativesMarket() {
  return (
    <>
      <Section subtitle="01 — WHAT IS A DERIVATIVE" title="Contracts whose value comes from something else">
        <div style={{ background: C.panel, border: '1px solid ' + C.border, borderRadius: 4, padding: 24 }}>
          <p style={{ fontSize: 11, color: C.textSec, lineHeight: 1.9, marginBottom: 20 }}>
            A derivative is a financial contra whose value is <span style={{ color: C.amber }}>derived from an underlying asset</span> — a stock, index, commodity, or currency. You are not buying the asset itself. You are buying a contract about its future price.
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 240px), 1fr))', gap: 12 }}>
            {[{ name: 'Futures', icon: String.fromCodePoint(0x1F4CB), color: C.blue, desc: 'A legal obligation to buy or sell at a set price on a futureate. Both buyer and seller MUST fulfil the contract.', example: 'You agree to buy 1 lot of Nifty at 22,000 on expiry regardless of where Nifty actually trades.' },
      ...
      <Section subtitle="02 — FUTURESs OPTIONS" title="Key differences at a glance">
        <div style={{ background: C.panel, border: '1px solid ' + C.border, borderRadius: 4, padding: 24 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: MONO, fontSize: 11 }}>
              ...
              <tbody>
                {[['Obligation', 'Must buy/sell', 'Right, not obligation'],
                  ...
                  ['Max profit (buyer)', 'Unlimited', 'Unted (calls)'],
      ...
      <Section subtitle="04 — HOW MARGINS WORKtle="Leverage — the double-edged sword">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 280px), 1fr))', gap: 16 }}>
          {[{ title: 'What is margin?', color: C.amber, content: 'In F&O you do not pay the full contract value. You pay a fraction called margin — typically 10-20%. This creates leverage. Nifty lot = 25 units x 22,000 = 5.5 lakh contract. Margin needed: ~1.1 lakh (20%). A 1% Nifty move = 5,500 gain/loss. That is a 5% move on your margin in oay.', warn: false },
     ...
     /Section>
        ...
        <div style={{ background: C.panel, border: '1px solid ' + C.border, borderRadius: 4, padding: 24 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 160px), 1fr))', gap: 12, marginBottom: 20 }}>
            {[['89%', 'Retail F&O traders lose money', C.red, 'SEBI Study 2023'],
              ['93%', 'Loss-making in options', C.red, 'SEBI Study 24'],
      ...
      <Section subtitle="06 — LEGITIMATE USE CASES" title="When derivatives actually make sense">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[{ icon: String.fromCodePoint(0x1F6E1), title: 'Hedginportfolio', color: C.green, desc: 'You hold 10 lakh of Nifty stocks. Before budget day you buy Nifty put options. If the market crashes, your puts profit and offset losses. This is what derivatives were designed for.' },
            ...
            { icon: String.fromCodePoint(0x26A0), title: 'Speculation — high risk', color: C.red, desc: 'Betting on short-term market direction with leverage. SEBI data shows 89% of retail traders lose. If you do this, use only mon you can afford to lose entirely.' },
          ].map(item => (
...
export default function HowMarketsWork() {
  ...
  return (
    <div style={{ background: C.nHeight: '100vh', fontFamily: MONO, color: C.text, overflowX: 'hidden' }}>
      <Navbar />
      <div style={{ maxWidth: 900, margin: '0 auto', padding: 'clamp(24px, 4vw, 52px) clamp(12px, 3vw, 24px)' }}>
        ...
        <div style={{ marginBottom: 64 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginTop: 32 }}>
            {[
              { label: 'Daily NSE turnover', value: 1200000, prefix: '₹', suffix: ' },
        ...
        <Section id="order-flow" subtitle="01 — TRADE LIFECYCLE" title="What happens when you click Buy?">
          ...
          <div style={{ marginTop: 16, fontSize: 11, color: C.textSec, lineHeight: 1.8, background: C.panel, border: `1px solid ${C.border}`, borderRadius: 3, padding: 16 }}>
            Your order travels from your app → broker's system → NSE's matching engine in milliseconds. The engine finds a seller w to sell at your price. Both sides are confirmed, and on T+1 (next trading day) the shares land in your demat account and money leaves your account.
          </div>
        </Section>
        ...
        <Section id="order-book" subtitle="02 — BID & ASK" title="The Order Book — where buyers meet sellers">
          <OrderBook />
          <div style={{ marginTop: 16, fontSize: 11, color: C.textSec, lineHeight: 1.8, background: C.panel, border: `1px solid ${C.border}`, borderRadius: 3dding: 16 }}>
        ...
        {/* 3. Supply demand */}
        <Section id="price-movement" subtitle="03 — PRICE DISCOVERY" title="How prices actua">
          <SupplyDemand />
          <div style={{ marginTop: 16, fontSize: 11, color: C.textSec, lineHeight: 1.8, background: C.panel, border: `1px solid ${C.border}`, borderRadius: 3, padding: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {ORDER_TYPES.map(o => (
              <div key={o.type} style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 4, padding: 20, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
                <div>
                  <div style={{ splay: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        ...
        <Section id="participants" subtitle="05 — WHO'S TRADING" title="Market participants and their role">
          <div style={{ display: 'flex', flexDirtion: 'column', gap: 10 }}>
            {PARTICIPANTS.map(p => (
        ...
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 4, padding: 28, textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <Link to="/glossary" style={{ background: 'transparent', color: C.amber, padding: '10px 24px', textDecorion: 'none', fontSize: 11, fontWeight: 700, letterSpacing: 1.5, borderRadius: 3, border: `1px solid ${C.amber}` }}>GLOSSARY →</Link>
          </div>
        </div>
